...
def test_add_or_reactivate_subscription_new():
    ...
    mock_persistence.__enter__.return_value.find_subscription_by_chat_and_address.return_value = None
    service = SubscriptionService(persistence_service=mock_persistence)

    service.add_or_reactivate_subscription(1, 10, "evening")

    mock_persistence.__enter__.return_value.create_subscription.assert_called_once_with(1, 10, "evening")
...
def test_add_or_reactivate_subscription_existing():
    ...
    mock_persistence = MagicMock()
    mock_persistence.__enter__.return_value.find_subscription_by_chat_and_address.return_value = {"id": 99}
    service = SubscriptionService(persistence_service=mock_persistence)
    ...
    mock_persistence.__enter__.return_value.reactivate_subscription.assert_called_once_with(99, "morning")
